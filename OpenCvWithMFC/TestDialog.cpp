...
{	
	...
	if (parentHwndOpenCV != NULL) 
	{
		...
		// Разрешение кадра камеры по умолчанию
		int minWidth = 1280;
		int minHeight = 1024;
		...
		// Убираем заголовок и предотвращаем перемещение при помощи стилей WS_CHILD и DS_MODALFRAME
...
BOOL TestDialog::OnInitDialog()
{
	...
	// Максимальное разрешение камеры: 1280x1024
...
UINT TestDialog::VideoThread(LPVOID pParam) 
{
	...
	while (!dlg->m_stopThread && dlg->m_videoCapture.isOpened()) 
	{
		...
		// NOTE: в MVS используются blob фильтры, нормализация и бинаризация. Также использовалась эрозия, но она закоментирована
