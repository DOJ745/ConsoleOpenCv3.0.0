...
/*
	...
	- nFeatures — чем больше значение, тем больше точек будет найдено, что увеличивает детализацию, но также увеличивает вычислительную нагрузку

	- scaleFactor — Коэффициент уменьшения масштаба изображения на каждом уровне пирамиды. 
		Значение, больше 1.0 (например, 1.2) означает, что каждое последующее изображение в пирамиде будет меньше предыдущего.
		Чем больше значение, тем быстрее уменьшается масштаб, что ускоряет обработку, но может привести к потере мелких деталей.

	- nLevels — уровни масштабов. Больше уровней позволяет находить ключевые точки на разных масштабах, что повышает устойчивость к изменениям масштаба.

	- edgeThreshold — Минимальное расстояние от края изображения, где могут быть обнаружены ключевые точки.
		Большие значения исключают точки, расположенные близко к краям, что снижает вероятность обнаружения "плохих" точек.

	- firstLevel — Индекс первого уровня пирамиды, с которого начинается поиск ключевых точек. 
		Обычно равен 0, что означает использование оригинального изображения. 

	- WTA_K — Количество точек в окрестности ключевой точки, которые сравниваются для генерации одного бита дескриптора.

	- scoreType — метод оценки: HARRIS_SCORE (устойчивее), FAST_SCORE (быстрее).

	- patchSize — Размер области вокруг ключевой точки, используемой для вычисления дескриптора.
		Большие значения позволяют учитывать больше деталей, но увеличивают вычислительную нагрузку.

	- fastThreshold — Пороговое значение для подавления слабых ключевых точек.
		Высокий порог приводит к меньшему количеству точек, но они будут более "выразительными".
*/
